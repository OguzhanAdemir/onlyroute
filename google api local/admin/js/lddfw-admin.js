jQuery(document).ready(function($) {
    $("body").on("click", "#lddfw_check_yandex_keys", function() {
        var lddfw_loading = $(this).attr("data-loading");
        var lddfw_title = $(this).attr("data-title");
        var lddfw_alert = $(this).attr("data-alert");
        var lddfw_yandex_api_key = "a8f7ce0e-61bf-4794-9901-3aba5c4b00da"; // Yandex Maps API key
        var lddfw_yandex_api_key_server = "043330dd-433a-44eb-ade5-3544b692f5e4"; // Geosuggest API key

        $("#lddfw_check_yandex_keys_wrap").show();
        if (lddfw_yandex_api_key == "" || lddfw_yandex_api_key_server == "") {
            $("#lddfw_check_yandex_keys_wrap").html(lddfw_alert);
            return false;
        }

        $("#lddfw_check_yandex_keys_wrap").html(lddfw_loading);
        $.post(lddfw_ajax.ajaxurl, {
            action: 'lddfw_ajax',
            lddfw_service: 'lddfw_check_yandex_keys',
            lddfw_obj_id: lddfw_yandex_api_key_server,
            lddfw_wpnonce: lddfw_nonce.nonce,
        }, function(data) {
            $("#lddfw_check_yandex_keys_wrap").html("");
            $("#lddfw_check_yandex_keys_wrap").append('<p class="title">' + lddfw_title + ' <b>' + lddfw_yandex_api_key_server + '</b></p>');
            $("#lddfw_check_yandex_keys_wrap").append(data);

            $("#lddfw_check_yandex_keys_wrap").append('<p class="title">' + lddfw_title + ' <b>' + lddfw_yandex_api_key + '</b></p>');
            $("#lddfw_check_yandex_keys_wrap").append('<p>Maps Embed API:</p><iframe width="450" height="250" style="border:0" loading="lazy" allowfullscreen referrerpolicy="no-referrer-when-downgrade" src="https://yandex.ru/map-widget/v1/?um=constructor:ddfw_test_map&source=constructor"></iframe>');
            $("#lddfw_check_yandex_keys_wrap").append('<p>Maps JavaScript API:</p><div style="width:450px;height:250px;" id="ddfw_test_map"></div><script src="https://api-maps.yandex.ru/2.1/?lang=en_RU&apikey=' + lddfw_yandex_api_key + '"></script>');

            function initMap() {
                ymaps.ready(function () {
                    var map = new ymaps.Map('ddfw_test_map', {
                        center: [41.85, -87.65],
                        zoom: 8
                    });
                    var directionsService = new ymaps.multiRouter.MultiRoute({
                        referencePoints: ['oklahoma city, ok', 'chicago, il'],
                        params: {
                            routingMode: 'auto'
                        }
                    }, {
                    ...
                    map.geoObjects.add(directionsService);
                    var geocoder = ymaps.geocode('indiana, in');
                    geocoder.then(function (res) {
                        var firstGeoObject = res.geoObjects.get(0);
                        map.setCenter(firstGeoObject.geometry.getCoordinates());
                        var marker = new ymaps.Placemark(firstGeoObject.geometry.getCoordinates());
