...
// All custom code for polyancity.by after here
    
document.addEventListener("DOMContentLoaded", function () {
    // Hide Google Maps and Waze navigation buttons
    const googleMapsButton = document.querySelector('a[href*="google.com/maps"]');
    const wazeButtons = document.querySelectorAll('a[href*="waze.com"]');

    // Hide Google Maps button
    if (googleMapsButton) {
        googleMapsButton.style.display = "none";
    }

    // Hide the first Waze button
    if (wazeButtons.length > 0) {
        wazeButtons[0].style.display = "none";
    }

    // Replace the second Waze button with a Yandex Maps button
    if (wazeButtons.length > 1) {
        // Define the pickup address with %20 for spaces
        const pickupAddress = "Поляна%20City,%20ул.%20Советская,%2025,%20г.Гродно,%20Беларусь";

        // Create a new Yandex Maps button
        const yandexButton = document.createElement("a");
        yandexButton.href = `https://yandex.com/maps/?text=${pickupAddress}`;
        yandexButton.target = "_blank";
        yandexButton.className = "lddfw_navigation col btn btn-secondary btn-block";
        yandexButton.innerHTML = `
            <svg aria-hidden="true" focusable="false" data-prefix="fab" data-icon="yandex" class="svg-inline--fa fa-yandex fa-w-14" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512">
                <path fill="currentColor" d="M448 32v448H0V32h448zM144 144v64h64v-64h-64zm0 128v160h64V272h-64zm128 0v160h64V272h-64z"/>
            </svg> ${wazeButtons[1].innerText}
        `;

        // Replace the second Waze button with the Yandex Maps button
        wazeButtons[1].parentNode.replaceChild(yandexButton, wazeButtons[1]);
    }
});